...
namespace df::net {
static inline UInt crc32(const UByte* data, Size len);

void net::Connection::receiveMessage()
{
    Address sender;
    const Size received = socket.receive(sender, msgBuf, BUFFER_SIZE);
    const UInt crc = crc32(msgBuf, received);
}
...
static inline UInt crc32(const UByte* data, Size len)
{
    ...
    for (Size i = 0; i < len; i++) {
        const UInt index = (crc ^ data[i]) & 0xff;
